const QUEST_COPY={
 ru:{title:'Награды за рост',tag:'ВАШ ОПЫТ — ВАШ ВЫБОР',balance:'Доступно',earned:'Заработано',spent:'Обменяно',demo:'Демонстрационный каталог. Промокоды не действуют. Предложения с партнёрами не согласованы.',rules:'100 XP за завершённую добровольную активность, включая историю. За повторный клуб — не чаще раза в месяц. Обязательные мероприятия не дают XP. Навыки и карьерный прогресс при обмене не расходуются. В демо завершение отмечает сотрудник; для реальных наград нужна проверка результата.',goal:'Коплю на',pick:'Копить',selected:'Моя цель',redeem:'Обменять',need:'Не хватает',ready:'Награда доступна',details:'Правила начисления',codes:'Мои промокоды',empty:'Здесь появятся ваши демо-промокоды после обмена.',confirm:'Подтвердить обмен',cancel:'Отмена',confirmHint:'С баланса будет списано',success:'Демо-промокод выдан. Найдите его в разделе «Мои промокоды».',error:'Не удалось выполнить действие. Обновите баланс или повторите попытку.',refresh:'Обновить баланс',awards:'История начислений',noAwards:'Завершите добровольную активность, чтобы получить первые XP.',badge:'Первый рубеж · 300 XP за развитие',toward:'Посмотреть шаги развития',invalid:'ДЕМО · НЕ ДЕЙСТВУЕТ'},
...
document.addEventListener('click',async event=>{
 ...
 try{
  ...
  else if(target.id==='quest-confirm'&&questPending){wallet=await fetchJson('/api/rewards/redeem',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({reward_id:questPending.reward,request_id:questPending.request})});if(state.auth===owner){questPending=null;questMessage='success';}}
